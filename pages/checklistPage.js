...
const ChecklistPage = ({ checklist_names_ }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user_email !== undefined) {
        ...
        try {
          await fetchUpdatedChecklists(user_email);
        } catch (error) {
  ...
  const submitChecklistRequest = async (
    ...
    if (res.ok) {
      ...
      console.error(
        `Failed to add new checklist: ${res.status} ${res.statusText}`
      );
    }
  };
    ...
    <div className="checklistPage">
      ...
      <div className="row justify-content-stretch h-100">
        <div className="col-12 span-all">
          {session.status === "authenticated" && (
            <>
              <h4 className={styles.heading}>
                Enter a prompt to create a checklist. For example, type in
                "Travel abroad"{" "}
              </h4>
              <input
                type="text"
                value={checklist_request}
                className={styles.searchBar}
                onChange={(e) => addChecklist(e.target.value)}
              />
            </>
          )}
          <button
            className="pl-0 pr-0 pt-1 pb-1 text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-0 focus:ring-gray-300 font-medium rounded-md text-sm px-3 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700 "
            onClick={() => {
          ...
          <br />
          {session.status !== "authenticated" && (
            <h4 className={styles.heading + " ml-5 ml-sm-3 ml-md-4"}>
              Sign in to create and save custom checklists!{" "}
            </h4>
          )}
...
async function fetchUpdatedChecklists(user_email) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to fetch updated checklists: ${res.status} ${res.statusText}`
      );
    }
